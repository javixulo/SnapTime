...
- Grid de miniaturas con indicadores visuales de score y estado (círculo de 16px: gris pendiente, verde correcto, #ffc107 sin sugerencia, azul claro con sugerencia). Al aceptar una sugerencia, el círculo cambia a azul oscuro (#1565C0).
...
- Al modificar la fecha, anota en los metadatos del archivo el valor original y la heurística responsable: campo `EXIF UserComment` (0x9286) en fotos, `QuickTime ©cmt` (UserData Comment) en vídeos. Escritura vía **Magick.NET-Q16-AnyCPU** (ImageMagick wrapper).
